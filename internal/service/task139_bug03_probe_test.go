...
func TestBug03_CancelledRouteReleasesResourcesAfterRestart(t *testing.T) {
	svc := newServiceWithYard(t); ctx:=context.Background(); sig, _, _, _, normal, _ := task139BugIDs(t, svc)
	r, err := svc.RequestRoute(ctx, RouteRequest{OriginSignalID:sig, TerminalSectionID:normal}); if err != nil { t.Fatal(err) }
	if _, err = svc.CancelRoute(ctx, r.Route.ID); err != nil { t.Fatal(err) }; if _, err = svc.Reconcile(ctx); err != nil { t.Fatal(err) }
	sec,_:=svc.GetSection(ctx,normal); if sec.LockedByRoute!="" { t.Fatalf("cancelled route left section locked by %s", sec.LockedByRoute) }
}
